...
まず, terraform.tfvars の `network_names` にネットワークキーとラベル名を定義します。
この `network_names` に追加したキーは Terraform によって新規作成され, 削除したキーは削除対象になります。
...
旧構成の `infrastructure_vms` などの出力は廃止し, 新構成では `vm_groups` と `vm_instances` を使用します。

```bash
# グループ単位で参照する場合
terraform output vm_groups

# group/vm キーで一意に参照する場合
terraform output vm_instances

# JSONで取得してAnsible連携などへ渡す場合
terraform output -json vm_instances
```

`vm_instances` のキーは `group/vm` 形式です。
例: `infrastructure/router`, `k8s/k8sctrlplane01`
...
terraform state list  # module.vms["infrastructure/router"] が消えていることを確認
```
...
# 個別リソース移行
terraform state mv 'xenorchestra_vm.router' 'module.vms["infrastructure/router"].xenorchestra_vm.this'
```
...
|オブジェクト名|VM種別|
|---|---|
|vm_groups.infrastructure|Infrastructure VM(router, mgmt-server, rhel-server, ubuntu-server, devserver)|
|vm_groups.devlinux|内部プライベートネットワークのみに接続された開発用VM|
|vm_groups.vmlinux|pool-wide networkに接続のみに接続された開発用VM|
|vm_groups.k8s|Kubernetes クラスタを構成するVM(extgw, frr*, k8sctrlplane*, k8sworker*)|
...
```makefile
new_server: prepare
  ${TERRAFORM} apply ${TERRAFORM_FLAGS} -target='module.vms["infrastructure/new_server"]' 2>&1 | tee $@.log

destroy-new_server: prepare
  ${TERRAFORM} destroy ${TERRAFORM_FLAGS} -target='module.vms["infrastructure/new_server"]' 2>&1 | tee $@.log
```
