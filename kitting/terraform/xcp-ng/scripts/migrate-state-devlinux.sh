...
#
# migrate-state-devlinux.sh
# Terraformのstateをdevlinux VMグループ分移行する
# Usage:
#  $ ./migrate-state-devlinux.sh [--dry-run]
# Options:
...
# ログファイル設定
LOG_FILE="scripts/migration-devlinux-$(date +%Y%m%d-%H%M%S).log"
...
echo "=== Devlinux VMs State Migration ===" | tee -a "$LOG_FILE"
...
# devlinux VMグループの一覧
VMS=("devlinux1" "devlinux2" "devlinux3" "devlinux4" "devlinux5")
