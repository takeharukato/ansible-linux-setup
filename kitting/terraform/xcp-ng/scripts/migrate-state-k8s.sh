...
#
# migrate-state-k8s.sh
# Kubernetes Lab VMグループ分Terraformのstateを移行する
# Usage:
#  $ ./migrate-state-k8s.sh [--dry-run]
# Options:
...
# ログファイル設定
LOG_FILE="scripts/migration-k8s-$(date +%Y%m%d-%H%M%S).log"
...
echo "=== Kubernetes Lab VMs State Migration ===" | tee -a "$LOG_FILE"
...
# 移行対象のK8s VMグループの一覧
VMS=("k8sctrlplane01" "k8sworker0101" "k8sworker0102" "frr01" "k8sctrlplane02" "k8sworker0201" "k8sworker0202" "frr02" "extgw")

for vm in "${VMS[@]}"; do
  echo "Processing VM: $vm" | tee -a "$LOG_FILE"

  # 既に移行済みであることを確認
  if terraform state list | grep -q "module.k8s_vms\[\"$vm\"\]"; then
  ...
  if terraform state list | grep -q "xenorchestra_cloud_config.$vm"; then
    CMD="terraform state mv 'xenorchestra_cloud_config.$vm' 'module.k8s_vms[\"$vm\"].xenorchestra_cloud_config.this'"
  ...
  if terraform state list | grep -q "xenorchestra_vm.$vm"; then
    CMD="terraform state mv 'xenorchestra_vm.$vm' 'module.k8s_vms[\"$vm\"].xenorchestra_vm.this'"
...
if [ "$DRY_RUN" = false ]; then
  ...
  terraform state list | grep "k8s_vms" | tee -a "$LOG_FILE"
  echo "" | tee -a "$LOG_FILE"
...
echo "1. Run: terraform plan -target='module.k8s_vms'" | tee -a "$LOG_FILE"
