...
for vm in "${VMS[@]}"; do
  ...
  # 既に移行済みであることを確認
  if terraform state list | grep -q "module.vms\[\"vmlinux/$vm\"\]"; then
  ...
  if terraform state list | grep -q "xenorchestra_cloud_config.$vm"; then
    CMD="terraform state mv 'xenorchestra_cloud_config.$vm' 'module.vms[\"vmlinux/$vm\"].xenorchestra_cloud_config.this'"
  ...
  if terraform state list | grep -q "xenorchestra_vm.$vm"; then
    CMD="terraform state mv 'xenorchestra_vm.$vm' 'module.vms[\"vmlinux/$vm\"].xenorchestra_vm.this'"
...
if [ "$DRY_RUN" = false ]; then
  ...
  terraform state list | grep 'module.vms\["vmlinux/' | tee -a "$LOG_FILE"
  echo "" | tee -a "$LOG_FILE"
