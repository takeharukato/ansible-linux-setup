...
#
# migrate-state-vmlinux.sh
# Vmlinux VMグループ分Terraformのstateを移行する
# Usage:
#  $ ./migrate-state-vmlinux.sh [--dry-run]
# Options:
...
# ログファイル設定
LOG_FILE="scripts/migration-vmlinux-$(date +%Y%m%d-%H%M%S).log"
...
echo "=== Vmlinux VMs State Migration ===" | tee -a "$LOG_FILE"
...
# 移行対象のvmlinux VMグループの一覧
VMS=("vmlinux1" "vmlinux2" "vmlinux3" "vmlinux4" "vmlinux5")

for vm in "${VMS[@]}"; do
  echo "Processing VM: $vm" | tee -a "$LOG_FILE"

  # 既に移行済みであることを確認
  if terraform state list | grep -q "module.vmlinux_vms\[\"$vm\"\]"; then
  ...
  if terraform state list | grep -q "xenorchestra_cloud_config.$vm"; then
    CMD="terraform state mv 'xenorchestra_cloud_config.$vm' 'module.vmlinux_vms[\"$vm\"].xenorchestra_cloud_config.this'"
  ...
  if terraform state list | grep -q "xenorchestra_vm.$vm"; then
    CMD="terraform state mv 'xenorchestra_vm.$vm' 'module.vmlinux_vms[\"$vm\"].xenorchestra_vm.this'"
...
if [ "$DRY_RUN" = false ]; then
  ...
  terraform state list | grep "vmlinux_vms" | tee -a "$LOG_FILE"
  echo "" | tee -a "$LOG_FILE"
...
echo "1. Run: terraform plan -target='module.vmlinux_vms'" | tee -a "$LOG_FILE"
