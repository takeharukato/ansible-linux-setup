...
#
# migrate-state-registry.sh
# Terraformのstateをregistry VMグループ分移行する
# Usage:
#  $ ./migrate-state-registry.sh [--dry-run]
# Options:
...
# ログファイル設定
LOG_FILE="scripts/migration-registry-$(date +%Y%m%d-%H%M%S).log"
...
echo "=== Registry VMs State Migration ===" | tee -a "$LOG_FILE"
...
for old in "${!VMS_MAP[@]}"; do
  ...
  if terraform state list | grep -q "module.vms\[\"registry/$new\"\]"; then
  ...
  if terraform state list | grep -q "xenorchestra_cloud_config.$old"; then
    CMD="terraform state mv 'xenorchestra_cloud_config.$old' 'module.vms[\"registry/$new\"].xenorchestra_cloud_config.this'"
  ...
  if terraform state list | grep -q "xenorchestra_vm.$old"; then
    CMD="terraform state mv 'xenorchestra_vm.$old' 'module.vms[\"registry/$new\"].xenorchestra_vm.this'"
...
if [ "$DRY_RUN" = false ]; then
  ...
  terraform state list | grep 'module.vms\["registry/' | tee -a "$LOG_FILE"
  echo "" | tee -a "$LOG_FILE"
