...
| 正式名称 | 略称 | 意味 |
| --- | --- | --- |
| RPM Package Manager | RPM | Red Hat系Linuxディストリビューションのパッケージ管理システム |
| Dandified YUM | DNF | YUMの後継として開発されたRPMベースのパッケージ管理ツール, RHEL 8以降で標準 |
| Yellowdog Updater Modified | YUM | RPMベースのパッケージ管理ツール, RHEL 7まで標準, DNFに置換 |
| GNU Privacy Guard | GPG | 電子署名と暗号化を行うオープンソース暗号化ソフトウェア |
| Extra Packages for Enterprise Linux | EPEL | RHEL系ディストリビューション向けの追加パッケージリポジトリ |
| Red Hat Enterprise Linux | RHEL | Red Hatが提供する商用Linuxディストリビューション |
| - | BaseOS | RHEL系ディストリビューションの基本パッケージを提供するコアリポジトリ |
| - | AppStream | RHEL系ディストリビューションのアプリケーションパッケージを提供するリポジトリ |
| - | cloud-init | クラウド環境でのLinuxインスタンス初期設定を自動化するツール |
| - | Kickstart | 設定ファイルを用いてRHELインストールプロセスを自動化する仕組み |
| - | Ansible | YAML形式で定義された自動化タスクを実行するIT自動化ツール |
| Uniform Resource Locator | URL | インターネット上のリソースの場所を示すアドレス表記 |
| Secure Sockets Layer | SSL | ネットワーク通信を暗号化するセキュリティプロトコル, 現在はTLSに移行 |
