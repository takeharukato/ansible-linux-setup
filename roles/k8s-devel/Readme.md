...
- **Python 言語版 Kubernetes client の二段階導入**: 標準 Python (例: python3) と開発用 Python (例: python3.12, RHEL のみ) の両方に Python 言語版 Kubernetes client を導入します。これにより, OS 標準の Python 環境と開発用の新しい Python 環境の両方で Kubernetes API を利用できます。
...
7. **Go 言語版 Kubernetes client 導入** (`install-go-client.yml`): `k8s_devel_go_client_enabled=true` の場合, Go 言語版 Kubernetes client を導入します。
   - 事前検証として `golang`/`golang-go`/`go-lang` パッケージの導入有無を確認し, 未導入の場合は設定誤りとして `fail` で停止します。
   - `{{ k8s_devel_go_client_work_dir }}` (既定: `/opt/k8s-devel/go-client`) ディレクトリを作成します。
...
| 変数名 | 既定値 | 説明 |
| --- | --- | --- |
| `k8s_devel_go_client_work_dir` | `"/opt/k8s-devel/go-client"` | Go 言語版 Kubernetes client 導入用作業ディレクトリ。`go mod init` と `go get` を実行し, `go.mod`, `go.sum` を配置するディレクトリです。実際の client-go ライブラリは Go のモジュールキャッシュにダウンロードされます。 |
...
`go_lang_version`, `go_lang_remove_existing_package` など Go 導入方式に関する変数は `go-lang-local` ロールで管理します。devel環境では `go-lang-local` ロールを playbook で実行して適用してください。設定方法と既定値は `roles/go-lang-local/Readme.md` を参照ください。
...
| ファイル | 用途 |
| --- | --- |
| `{{ k8s_devel_go_client_work_dir }}/go.mod` | `go mod init` 実行時に生成される Go モジュール定義ファイルです。 |
| `{{ k8s_devel_go_client_work_dir }}/go.sum` | `go get` 実行時に生成または更新される依存関係チェックサムファイルです。 |
| `kubernetes` Python パッケージ | Python 言語版 Kubernetes client 導入時に pip で導入されます。導入先は Ubuntu 24.04 では通常 `/usr/local/lib/python3.12/dist-packages/kubernetes/`, RHEL 9.6 では通常 `/usr/local/lib/python3.9/site-packages/kubernetes/` です。 |
...
# Python 言語版 Kubernetes clientの版数として31.0.0を指定する
...
### 2. Python 言語版 Kubernetes client 導入の確認 (Ubuntu 24.04)
...
### 3. Python 言語版 Kubernetes client 導入の確認 (RHEL 9.6 - 標準 Python)
...
### 4. Python 言語版 Kubernetes client 導入の確認 (RHEL 9.6 - 開発用 Python)
...
- Go 言語版 Kubernetes client 導入で失敗する場合は, `golang` の導入有無とネットワーク疎通を確認してください。
- Python 言語版 Kubernetes client 導入で失敗する場合は, pip 実行環境と外部リポジトリアクセス可否を確認してください。
