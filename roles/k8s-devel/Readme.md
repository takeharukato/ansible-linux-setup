...
3. `post-load-params-python-client.yml` で Python client 導入用の実効変数 (`k8s_devel_python_prereq_packages_effective`, `k8s_devel_python_pip_executable_effective`) を確定します。
4. `package.yml` で前提パッケージ, Python client, Go client, C client(将来サポート予定) を順に処理します。
...
| `k8s_python_packages_version` | `"3.12"` | 開発用 Python バージョン。Ubuntu では標準 Python が該当バージョンのため参照のみ, RHEL では `python3.12`, `python3.12-pip` 等の版数指定パッケージ名に使用されます。`vars/packages-ubuntu.yml`, `vars/packages-rhel.yml` で定義されます。 |
...
- Python client は以下の2段階で導入されます:
  1. **標準 Python (python3)** への導入: `pip3` を使用して `kubernetes<python_client_version>` を導入します。
  2. **開発用 Python** への導入: `k8s_python_packages_version` が定義されている場合, 該当バージョンの `pip` (RHEL: `pip3.12` 等) を使用して追加導入します。Ubuntu では標準 Python が該当バージョンのため実質的に冪等性により重複インストールとなります。
  ...
  - RHEL 9.6 では標準 Python 3.9 と開発用 Python 3.12 の両方に Kubernetes client が導入されます。
...
- `k8s_devel_python_client_enabled=true` で pip から `kubernetes` パッケージが導入されます。
  - Ubuntu: 標準 Python (python3.12) に Kubernetes client が導入されます。
  - RHEL: 標準 Python (python3.9) と開発用 Python (python3.12) の両方に Kubernetes client が導入されます。
...
- 対象Kubernetes ノードが inventory 上で `k8s_ctrl_plane`, `k8s_worker`, `devel`, `internal_devel` のいずれかのグループに所属していること。
...
**RHEL 9.6 の場合:**
```bash
# 標準 Python (python3.9) の確認
pip3 list | grep kubernetes
python3 -m pip show kubernetes
python3 -c 'import kubernetes; print(kubernetes.__version__)'

# 開発用 Python (python3.12) の確認
pip3.12 list | grep kubernetes
python3.12 -m pip show kubernetes
...
- パス確認の目安は以下です。
  ...
  - RHEL 9.6 (標準): `/usr/local/lib/python3.9/site-packages/kubernetes/__init__.py`
  - RHEL 9.6 (開発用): `/usr/local/lib/python3.12/site-packages/kubernetes/__init__.py`
- 版数未指定時は `k8s_major_minor` の マイナーバージョンに合わせた版数のPython clientが導入されます。
- RHEL では標準 Python と開発用 Python の両方に同一版数の Kubernetes client が導入されることを確認してくださいes/__init__.py`
