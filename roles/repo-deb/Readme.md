...
| Debian Package | DEB | Debian系Linuxで使用されるソフトウェアパッケージファイル形式 |
| - | deb822 | Debian制御ファイルの構造化テキスト形式, APTソース定義の新形式 |
| GNU Privacy Guard | GPG | 電子署名と暗号化を行うオープンソース暗号化ソフトウェア |
| Transport Layer Security | TLS | インターネット通信を暗号化するセキュリティプロトコル, SSLの後継 |
