...
- [elastic-agent-k8s ロール](#elastic-agent-k8s-ロール)
  ...
  - [用語](#用語)
  - [概要](#概要)
    - [Kubernetes API監査ログ収集用Elastic Agentとの役割分担](#kubernetes-api監査ログ収集用elastic-agentとの役割分担)
  - [前提条件](#前提条件)
  - [実行方法](#実行方法)
  - [主要変数](#主要変数)
...
本ロールは`k8s_cluster`構成種別のElastic Agentを導入し, kube-state-metricsを使用したKubernetesクラスタ状態情報を収集します。Kubernetes API監査ログは本ロールでは収集せず, `elastic-agent-k8s-audit`ロールが`k8s_audit`構成種別の別Helm releaseとしてcontrol-planeノードへDaemonSetを導入して収集します。

この分離により, 本ロールのKubernetes統合Package PolicyではAudit入力を無効化し, Audit用Package Policyだけで`audit-logs-filestream`を有効化して重複収集を防止します。
## 前提条件
