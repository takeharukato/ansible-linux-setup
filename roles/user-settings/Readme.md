...
| `user_settings_systemd_ssh_agent_skel` | `false` | systemd ユーザー機能を使用した ssh-agent の設定ファイルを作成する場合は `true` を指定します。`true` を指定すると `/etc/skel/.config/systemd/user/` 配下に `ssh-agent.service` と `ssh-agent.socket` が配置され, `.zprofile` と `.bash_profile` で `SSH_AUTH_SOCK` が systemd のソケットを参照するように設定されます。SSH 接続経由のログイン時は SSH agent forwarding を維持するため `SSH_AUTH_SOCK` を上書きしません。有効化するには, ユーザ側で `systemctl --user enable --now ssh-agent.socket` を実行する必要があります。 |
...
| `mdns_host_list` | `[]` | `.ssh/config` に `Host <name>.local` を追記するためのホスト一覧。未定義または空リストの場合は mDNS 向け Host エントリを出力しません。 |
| `dns_host_list` | `[]` | `.ssh/config` に `Host <name>.<dns_domain>` を追記するためのホスト一覧。未定義または空リストの場合は DNS 向け Host エントリを出力しません。 |
...
#### `.ssh/config`ファイルにDNSホストを登録するための設定

`dns_host_list`変数に以下の要素からなる辞書のリストを記述することで, ユーザの`.ssh/config`ファイルに `ホスト名.ドメイン名` の Host エントリを追記できます。
...
`user_settings_systemd_ssh_agent_skel` が `true` に設定されている場合, このロールは systemd ユーザユニット (ssh-agent.socket, ssh-agent.service) を `/etc/skel/.config/systemd/user/` に配置します。

### ユーザでの有効化手順

新規ユーザの作成後, 各ユーザは以下のコマンドで systemd ssh-agent を有効化する必要があります:

```bash
systemctl --user enable --now ssh-agent.socket
```

このコマンドにより ssh-agent がユーザログイン時に自動起動するようになります。

### SSH エージェントフォワーディングとの共存

bash (.bash_profile), zsh (.zprofile) の設定では, SSH 接続経由でログインした場合 (SSH_CONNECTION 環境変数が設定されている場合) は SSH_AUTH_SOCK を上書きしないため, SSH エージェントフォワーディングが正常に機能します。

ローカルログイン時のみ, systemd ssh-agent のソケット (`$XDG_RUNTIME_DIR/ssh-agent.socket`) が SSH_AUTH_SOCK に設定されます。
