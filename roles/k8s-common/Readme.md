...
| `templates/containerd-registry-hosts.toml.j2` | `/etc/containerd/certs.d/<endpoint>/hosts.toml` | containerd が参照するレジストリ endpoint 設定。`k8s_containerd_registry_endpoints` が非空の場合に `<endpoint>` ごとに生成します。 `<endpoint>`は, `<ホスト名, または, IPアドレス>:<ポート>`形式の文字列です。|
