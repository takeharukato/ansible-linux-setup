...
本ロールは, Kubernetesの各ノード共通の前提条件を整えるロールです。

## 目次

- [k8s-common ロール](#k8s-common-ロール)
  - [目次](#目次)
  - [用語](#用語)
  - [概要](#概要)
  - [前提条件](#前提条件)
  ...
  - [主要変数](#主要変数)
    ...
    - [ネットワーク設定](#ネットワーク設定)
    - [kubelet 設定](#kubelet-設定)
    - [ファイアウォール設定](#ファイアウォール設定)
    - [Python パッケージ設定](#python-パッケージ設定)
    - [containerd レジストリ設定](#containerd-レジストリ設定)
    - [kubectl 補完設定](#kubectl-補完設定)
    - [その他ディレクトリ設定](#その他ディレクトリ設定)
    - [Cilium BGP Control Plane設定](#cilium-bgp-control-plane設定)
      - [Cilium BGP Control Plane設定関連変数](#cilium-bgp-control-plane設定関連変数)
      - [`k8s_bgp`変数の`address_families`の要素を辞書として指定する場合の指定方法](#k8s_bgp変数のaddress_familiesの要素を辞書として指定する場合の指定方法)
  - [設定例](#設定例)
    - [パターン 1: 基本的な設定 (UFW, 補完有効, ファイアウォール有効)](#パターン-1-基本的な設定-ufw-補完有効-ファイアウォール有効)
    - [パターン 2: Cilium BGP Control Plane 有効化](#パターン-2-cilium-bgp-control-plane-有効化)
    - [パターン 3: RHEL/CentOS 系向け (firewalld, Python開発環境有効)](#パターン-3-rhelcentos-系向け-firewalld-python開発環境有効)
    - [パターン 4: 複数 NIC 構成, CPU シールディング有効](#パターン-4-複数-nic-構成-cpu-シールディング有効)
    - [パターン 5: containerd レジストリ endpoint 設定](#パターン-5-containerd-レジストリ-endpoint-設定)
  - [テンプレートと生成ファイル](#テンプレートと生成ファイル)
  - [実行フロー](#実行フロー)
    - [ステップ1: パラメータと設定読み込み](#ステップ1-パラメータと設定読み込み)
    - [ステップ2: ディレクトリ, ファイルシステム準備](#ステップ2-ディレクトリ-ファイルシステム準備)
    - [ステップ3: パッケージインストール](#ステップ3-パッケージインストール)
    - [ステップ4: ネットワーク, ファイアウォール, 補完設定](#ステップ4-ネットワーク-ファイアウォール-補完設定)
    - [ステップ5: kubelet と CPU シールディング設定](#ステップ5-kubelet-と-cpu-シールディング設定)
    - [ステップ6: kubeconfig ツール配布](#ステップ6-kubeconfig-ツール配布)
    - [ステップ7: カーネル, sysctl, containerd 設定とリブート](#ステップ7-カーネル-sysctl-containerd-設定とリブート)
    - [ステップ8: ユーザ, グループ管理](#ステップ8-ユーザ-グループ管理)
    - [デフォルト動作](#デフォルト動作)
    - [OS 差異](#os-差異)
      - [Ubuntu (Debian 系)](#ubuntu-debian-系)
      - [Red Hat 系 (CentOS/RHEL)](#red-hat-系-centosrhel)
  - [検証ポイント](#検証ポイント)
    - [パターン A: 基本的な環境検証](#パターン-a-基本的な環境検証)
    - [パターン B: ファイアウォール, ネットワーク検証](#パターン-b-ファイアウォール-ネットワーク検証)
    - [パターン C: containerd レジストリ設定検証](#パターン-c-containerd-レジストリ設定検証)
    - [パターン D: kubectl 補完, bash/zsh 設定確認](#パターン-d-kubectl-補完-bashzsh-設定確認)
    - [パターン E: Cilium BGP Control Plane 環境での検証](#パターン-e-cilium-bgp-control-plane-環境での検証)
    - [パターン F: ユーザ, 公開鍵管理確認](#パターン-f-ユーザ-公開鍵管理確認)
  - [トラブルシューティング](#トラブルシューティング)
    - [kubelet が起動しない / ステータスが failed](#kubelet-が起動しない--ステータスが-failed)
    - [swap が有効のままになっている](#swap-が有効のままになっている)
    - [kubectl 補完が機能しない](#kubectl-補完が機能しない)
    - [Cilium BGP neighbors が接続できない](#cilium-bgp-neighbors-が接続できない)
  - [注意事項](#注意事項)
  - [参考資料](#参考資料)
    - [公式ドキュメント](#公式ドキュメント)
...
| Address Family Identifier | AFI | 通信先アドレス種別を示す識別子。 |
...
本ロールは, k8s-common に関する設定処理を実施します。
...
### パターン 5: containerd レジストリ endpoint 設定
...
container_registry_endpoints:
  ...
  - endpoint: "devserver.example.org:5050"
    scheme: "https"
    skip_verify: true
k8s_containerd_registry_plain_http: true
```
...
| --- | --- | --- |
| `create-uniq-kubeconfig.py.j2` | `{{ k8s_create_unique_kubeconfig_script_path }}` (既定: `{{ k8s_create_unique_kubeconfig_script_path }}`) | クラスタ識別子を保持した重複のない kubeconfig を生成する補助スクリプトです。 |
| `Readme-uniq-kubeconfig-JP.md` | `{{ k8s_node_setup_tools_docs_dir }}/Readme-uniq-kubeconfig-JP.md` (既定: `{{ k8s_node_setup_tools_docs_dir }}/Readme-uniq-kubeconfig-JP.md`) | kubeconfig 生成手順と運用時の注意点を説明する利用者向けドキュメントです。 |
| `systemd-kubepod-cpuset.conf.j2` | `/etc/systemd/system/kubepods.slice.d/40-cpuset.conf` (既定: `/etc/systemd/system/kubepods.slice.d/40-cpuset.conf`) | kubepods.slice の CPU 割り当てを固定し, 制御プレーン負荷の干渉を抑える systemd 設定です。 |
| `rpfilter-bypass-ensure.sh.j2` | `/usr/local/sbin/rpfilter-bypass-ensure.sh` (既定: `/usr/local/sbin/rpfilter-bypass-ensure.sh`) | 逆引き経路フィルタによる誤遮断を回避するための例外ルールを適用する保守スクリプトです。 |
...
12. `config.yml` がカーネルモジュール, sysctl, containerd 設定を反映し, SystemdCgroup を有効化した後に, `k8s_containerd_registry_endpoints` で指定した endpoint ごとの `hosts.toml` を `/etc/containerd/certs.d` 配下へ生成します。最後に containerd を再起動し, ホストをリブートします。
...
### パターン C: containerd レジストリ設定検証

**目的**: containerd が指定 endpoint を参照し, pull/resolve/push を許可する `hosts.toml` が生成されていること, K8sノードからendpointで指定したコンテナレジストリにアクセス可能であることを確認。
...
```bash
$ grep -n 'config_path = "/etc/containerd/certs.d"' /etc/containerd/c
onfig.toml
274:      config_path = "/etc/containerd/certs.d"
$ ls -la /etc/containerd/certs.d
合計 16
drwxr-xr-x 4 root root 4096  7月 11 08:31 .
drwxr-xr-x 3 root root 4096  7月 11 08:31 ..
drwxr-xr-x 2 root root 4096  7月 11 08:31 registry.local:5000
$ cat /etc/containerd/certs.d/registry.local:5000/hosts.toml
#
#  -*- coding:utf-8 mode:toml -*-
# This file is generated by ansible.
# last update: 2026-07-11 08:20:38 JST
#
#
# registry.local:5000用のコンテナレジストリ設定
#
# コンテナレジストリサーバ
...
### パターン D: kubectl 補完, bash/zsh 設定確認
...
### パターン E: Cilium BGP Control Plane 環境での検証
...
# CiliumBGPPeeringPolicy が適用されていることを確認
...
# authorized_keys が正しくソート, 重複排除されていることを確認
...
1. **Node IP が解決できない**: `/etc/default/kubelet` に `--node-ip=<IP>` が正しく設定されていることを確認。未設定の場合, `k8s_kubelet_nic` の NIC に割り当てられた IP を `host_vars` に指定。
...
**対処**: `config-disable-swap.yml` タスクをスキップしていないことを確認し, ロール全体を再実行。
...
- **GitHub 鍵取得失敗**: ネットワーク遅延等で GitHub からの鍵取得がタイムアウトする場合, `k8s_operator_authorized_key_list` で事前に鍵を登録, またはロール実行後に手動で `~/.ssh/authorized_keys` に追記してください。
...
- **パッケージマネージャー** (APT / Yum / Dnf): 本ロール内でパッケージ更新を実施するため, パッケージマネージャーのロック状態 (別プロセスが apt/yum を実行中) がないことを確認してください。
