...
7. **必須変数の設定**: 以下の変数を`group_vars/all`または`host_vars/`で必ず設定すること。設定されていない場合, ロール実行時にfailで停止。
   ...
   - `ldap_domain`: LDAPドメイン名(例: `example.com`, 空文字列不可)
...
| 変数名 | 既定値 | 説明 |
| --- | --- | --- |
| `ldap_organization` | `""` (必須) | LDAP組織名。OpenLDAPコンテナの`LDAP_ORGANISATION`環境変数として設定されます。未定義または空文字列の場合, ロール実行時にfailで停止します |
| `ldap_domain` | `""` (必須) | LDAPドメイン名。OpenLDAPコンテナの`LDAP_DOMAIN`環境変数として設定され, DC構成要素に使用されます。未定義または空文字列の場合, ロール実行時にfailで停止します |
| `ldap_admin_password` | `admin` | LDAP管理者(cn=admin)のパスワード。OpenLDAPコンテナの`LDAP_ADMIN_PASSWORD`環境変数として設定される |
| `ldap_admin_port` | `10443` | phpLDAPadmin Web UI のHTTPS公開ポート番号 |
...
ldap_domain: "example.com"             # ドメイン名 ( 必須 )
...
ldap_domain: "ldap.example.com"        # ドメイン名を上書き ( オプション )
