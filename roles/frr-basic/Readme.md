...
| Classless Inter-Domain Routing | CIDR | IP アドレスとネットワークプレフィックス長を組み合わせた表記法 (例: 10.0.0.0/8)。柔軟なネットワーク分割を可能にする。 |
...
| systemd | - | Linux システムの初期化とサービス管理を行うシステム・サービスマネージャ。起動プロセスの並列化と依存関係管理を提供する。 |
