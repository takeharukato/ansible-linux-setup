...
このロールはKubernetesの各ノードで利用する `kubeconfig` を生成, 統合, 配布し, オペレータ (`k8s_operator_user` 変数で定義, `kube` ユーザ) 用ホームディレクトリと `/etc/kubernetes` の両方を一貫した内容に保ちます。コントロールプレーンノード毎に証明書埋め込み版 `kubeconfig` を生成し, kubeconfigファイル結合ツール(`create-uniq-kubeconfig.py`) で複数クラスタコンテキストを統合した上で, 各ワーカーノードへも同じ内容を展開します。
...
| `/etc/kubernetes/merged-kubeconfig.conf` | 全Kubernetes ノード | 全コントロールプレーンノードのコンテキストを統合した `kubeconfig`。コントロールプレーンノード/ワーカーノード共通で参照。|
...
| `~{{ k8s_operator_user }}/.kube/merged-kubeconfig.conf` | 全Kubernetes ノード | `/etc/kubernetes/merged-kubeconfig.conf` のコピー。オペレータユーザが直接参照。|
| `~{{ k8s_operator_user }}/.kube/config -> merged-kubeconfig.conf` | 全Kubernetes ノード | 統合 `kubeconfig` への相対シンボリックリンク。既存ファイルは `config-default` にバックアップ。|
