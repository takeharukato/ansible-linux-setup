...
- 本ロールは実行ユーザごとの実効作業ディレクトリ(`/tmp/go-build-<USER>`)を内部で選択して利用する。
- 実効作業ディレクトリへの書き込み確認に失敗した場合は, フォールバック先(`/tmp/go-build-fallback-<USER>`)へ切り替えて継続する。
- 成果物出力先は入力変数で受け取らず, 実効作業ディレクトリ配下の `output` を内部で使用する。
...
4. ビルド用コンテナイメージを Dockerfile から作成する。
5. コンテナ内で Go パッケージ(deb/rpm)を構築する。
6. 成果物の存在確認と版数確認を行う。
7. 構築済みパッケージを構築ホストから制御ノードに回収する。
8. 制御ノードから対象ホストへコピーして導入する。
9. 導入後の `go version` で版数一致を確認する。
