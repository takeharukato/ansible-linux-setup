...
- [Logstash ロール](#logstash-ロール)
  ...
  - [前提条件](#前提条件)
    ...
    - [Kubernetes API監査ログの転送](#kubernetes-api監査ログの転送)
  - [実行方法](#実行方法)
  ...
  - [検証ポイント](#検証ポイント)
    ...
    - [検証コマンドと期待結果](#検証コマンドと期待結果)
      ...
      - [3. Kubernetes API監査ログData Stream確認](#3-kubernetes-api監査ログdata-stream確認)
    - [異常時の確認項目](#異常時の確認項目)
...
- コンテナイメージは `docker.elastic.co/logstash/logstash:8.19.19` を使用すること。
...
### Kubernetes API監査ログの転送

`elastic-agent-k8s-audit`が収集する`kubernetes.audit_logs`は, Fleet Outputから本ロールの`elastic_agent`入力へ送信されます。Logstashの名前空間振り分けフィルタは`kubernetes.state_*`及び`kubernetes.event`だけを対象とするため, `kubernetes.audit_logs`の`data_stream.namespace`はFleet Bootstrapが設定した値を変更しません。

既定構成ではAuditイベントを`data_stream.dataset: kubernetes.audit_logs`, `data_stream.namespace: k8s_system`のままElasticsearchへ転送し, `logs-kubernetes.audit_logs-k8s_system`へ格納します。
...
#### 3. Kubernetes API監査ログData Stream確認

Kibana Dev Tools又はElasticsearch APIから次を確認します。
...
`data_stream.dataset`が`kubernetes.audit_logs`, `data_stream.namespace`が`k8s_system`であり, `kubernetes.audit.verb`, `kubernetes.audit.user`, `kubernetes.audit.objectRef`, `kubernetes.audit.responseStatus`等が構造化されていることを確認します。
...
68e1385c2532   docker.elastic.co/logstash/logstash:8.19.19   "/usr/local/bin/dock…"   2 hours ago   Up 2 hours   0.0.0.0:5044->5044/tcp, [::]:5044->5044/tcp, 0.0.0.0:9600->9600/tcp, [::]:9600->9600/tcp   logstash
$ docker logs --tail 200 logstash  2>&1 | grep -E '"log.level"[[:space:]]*:[[:space:]]*"(WARN|ERROR|FATAL)"'
$ docker compose -f /srv/logstash/docker-compose.yml config
name: logstash
services:
...
$ docker ps --filter name=logstash
CONTAINER ID   IMAGE                                        COMMAND                  CREATED       STATUS       PORTS                                                                                      NAMES
68e1385c2532   docker.elastic.co/logstash/logstash:8.19.19   "/usr/local/bin/dock…"   3 hours ago   Up 3 hours   0.0.0.0:5044->5044/tcp, [::]:5044->5044/tcp, 0.0.0.0:9600->9600/tcp, [::]:9600->9600/tcp   logstash
$ ss -ltnp | grep ':9600 '
LISTEN 0      4096         0.0.0.0:9600       0.0.0.0:*
LISTEN 0      4096            [::]:9600          [::]:*
