...
- [docker-network-elastic-stack ロール](#docker-network-elastic-stack-ロール)
  ...
  - [用語](#用語)
  - [概要](#概要)
    - [Kubernetes API監査ログ収集との関係](#kubernetes-api監査ログ収集との関係)
  - [前提条件](#前提条件)
  - [実行方法](#実行方法)
  - [主要変数](#主要変数)
...
### Kubernetes API監査ログ収集との関係

`elastic-agent-k8s-audit`ロールがKubernetesクラスタ上へ配置するElastic Agent Podは, 本ロールが作成するDockerブリッジネットワーク`elastic-backend`へ直接接続しません。Audit用Elastic AgentはFleet Server及びLogstashの対象ホスト公開エンドポイントへKubernetesクラスタ側から接続し, backend側ではFleet Server, Logstash, Kibana及びElasticsearchコンテナ間通信に本ロールのDockerブリッジネットワークを使用します。

このため, Kubernetes API監査ログ機能の追加によって本ロールのCIDR, iptables又はip6tables設定を追加変更する必要はありません。
## 前提条件
