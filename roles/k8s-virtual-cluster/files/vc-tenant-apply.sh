...
# テナント内へリソースをkubectl applyで配置するスクリプト
# VirtualClusterテナント環境に対してKubernetesマニフェストを適用します。
...
usage() {
  cat <<EOF
使用方法: $SCRIPT_NAME <テナント名> [-f マニフェストファイル] [kubectl applyオプション...]

説明:
  VirtualClusterテナント内へリソースをapplyします。
  テナント名から自動的に管理namespace(vc-manager-XXXXX-tenant-name)を取得し、
  そのnamespaceへkubectl applyコマンドを実行します。
...
オプション:
  ...
  [その他のオプション]      kubectlのapplyコマンドに渡されます

実行例:
  # YAMLファイルから適用
  $SCRIPT_NAME tenant-alpha -f pod-manifest.yaml

  # 複数ファイルの適用
  $SCRIPT_NAME tenant-alpha -f deployment.yaml -f service.yaml

  # ドライランオプション付き
  $SCRIPT_NAME tenant-alpha -f pod.yaml --dry-run=client
EOF
}
...
main() {
  ...
  # kubectlオプションを$tenment_nsに指定してapplyを実行
  kubectl -n "$tenant_ns" apply "$@"
}
